...
% Only for cartesian pose (position+orientation) control and joint damper included.
...
%% PD CONTROLLER
K_x = [80*eye(3), zeros(3,3); zeros(3,3), 5*eye(3)]; 
D_x = [20*eye(3), zeros(3,3); zeros(3,3), 0.1*eye(3)];
D_q = 1 * eye(7);
...
while ((vrep.simxGetConnectionId(clientID) ~= -1) && (t < 5))  % vrep connection is still active    
    ...
    %% 2. set desired x (only once, from [0.5, -0.2, 0.5] to [0.4, -0.3, 0.4])
    if ~exist('dx','var')
        dx = eePos - 0.1;
    end
    if ~exist('dEuler', 'var')
    ...
    geometryJacobian = [J(4:6, :);J(1:3, :)];   % geometry jacobian, note that 4-6 for translation, 1-3 for rotation
	
	%% PD-based setpoint control, with joint motion damper
	% NOTE:
    ...
    tau = geometryJacobian'*(K_x*e-D_x*geometryJacobian*qdot) - D_q*qdot + G; % + C;
...
savefig('results\6dof\setpoint_cartpos_error.fig');
...
title('End-effector Orientation Error of Setpoint Controller');
savefig('results\6dof\setpoint_cartrot_error.fig');
...
savefig('results\6dof\setpoint_cartpos_path.fig');

plotJointFigures(4, T, U, "Joint Torque of Setpoint Controller", 'Torque[Nm]')
savefig('results\6dof\setpoint_joint_torque.fig');
plotJointFigures(5, T, Q, "Joint Configuration of Setpoint Controller", 'Angle[rad]')
savefig('results\6dof\setpoint_joint_config.fig');
plotJointFigures(6, T, QDOT, "Joint Velocity of Setpoint Controller", 'Velocity[rad/s]')
savefig('results\6dof\setpoint_joint_vel.fig');
