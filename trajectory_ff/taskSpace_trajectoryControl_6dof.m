...
% Cartesian pose (position+orientation) control is implemented.
% => tau = M*qdotdot_c + C + G;
...
% max error: 2.5mm
K_x = [2000*eye(3), zeros(3,3); zeros(3,3), 50*eye(3)];   
D_x = [89*eye(3), zeros(3,3); zeros(3,3), 15*eye(3)];     
D_q = 0.5 * eye(7);
...
while ((vrep.simxGetConnectionId(clientID) ~= -1) && (t < 5))  % vrep connection is still active    
    ...
    % % transJacob_inv = (M \ transJacob') / ((transJacob / M) * transJacob');
    % ***** (2) ordinary Moore-Penrose of geometric Jacobian *****
...
savefig('results\6dof\trajectory_cartpos_error.fig');
...
savefig('results\6dof\trajectory_cartpos_path.fig');
...
title('End-effector Orientation Error of Trajectory Controller');
savefig('results\6dof\trajectory_cartrot_error.fig');

plotJointFigures(4, T, U, "Joint Torque of Setpoint Controller", 'Torque[Nm]')
savefig('results\6dof\trajectory_joint_torque.fig');
plotJointFigures(5, T, Q, "Joint Configuration of Setpoint Controller", 'Angle[rad]')
savefig('results\6dof\trajectory_joint_config.fig');
plotJointFigures(6, T, QDOT, "Joint Velocity of Setpoint Controller", 'Velocity[rad/s]')
savefig('results\6dof\trajectory_joint_vel.fig');
