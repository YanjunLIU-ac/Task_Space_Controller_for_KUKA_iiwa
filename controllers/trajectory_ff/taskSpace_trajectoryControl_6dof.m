...
% max error: 2.5mm
K_x = [2000*eye(3), zeros(3,3); zeros(3,3), 5000*eye(3)];   
D_x = [89*eye(3), zeros(3,3); zeros(3,3), 141*eye(3)];     
D_q = 0.5 * eye(7);
...
savefig('results\6dof\trajff_cartpos_error.fig');
...
savefig('results\6dof\trajff_cartpos_path.fig');
...
savefig('results\6dof\trajff_cartrot_error.fig');

plotJointFigures(4, T, U, "Joint Torque of Trajectory Controller", 'Torque[Nm]')
savefig('results\6dof\trajff_joint_torque.fig');
plotJointFigures(5, T, Q, "Joint Configuration of Trajectory Controller", 'Angle[rad]')
savefig('results\6dof\trajff_joint_config.fig');
plotJointFigures(6, T, QDOT, "Joint Velocity of Trajectory Controller", 'Velocity[rad/s]')
savefig('results\6dof\trajff_joint_vel.fig');
