...
while ((vrep.simxGetConnectionId(clientID) ~= -1) && (t<5))  % vrep connection is still active    
	...
	% % N = eye(7) - transJacob_inv * transJacob;
	% refence joint-space velocity/acceleration and sliding mode vector
	...
	%% 3-b. integrate torque for sliding mode control
	% using saturation function
	tau = M * rqdotdot + C + G - K_s * min(0.05, max(-0.05, s));
...
savefig('results\3dof\trajsat_cartpos_error.fig');
