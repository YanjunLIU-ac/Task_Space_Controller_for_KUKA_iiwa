...
K_x1 = [800*eye(3), zeros(3); zeros(3), 800*eye(3)];
% for D part in second-order inverse kinematics
D_x2 = [57*eye(3), zeros(3); zeros(3), 57*eye(3)];
% for P part in second-order inverse kinematics
K_x2 = [800*eye(3), zeros(3); zeros(3), 800*eye(3)];
...
title({'End-effector Position Error'; 'of Trajectory Controller (SMC-sgn)'});
savefig('results\6dof\trajsgn_cartpos_error.fig');
...
savefig('results\6dof\trajsgn_cartpos_path.fig');
...
title({'End-effector Orientation Error of'; 'Trajectory Controller (SMC-sgn)'});
savefig('results\6dof\trajsgn_cartrot_error.fig');

plotJointFigures(4, T, U, "Joint Torque of Trajectory Controller (SMC-sgn)", 'Torque[Nm]')
savefig('results\6dof\trajsgn_joint_torque.fig');
plotJointFigures(5, T, Q, "Joint Configuration of Trajectory Controller (SMC-sgn)", 'Angle[rad]')
savefig('results\6dof\trajsgn_joint_config.fig');
plotJointFigures(6, T, QDOT, "Joint Velocity of Trajectory Controller (SMC-sgn)", 'Velocity[rad/s]')
savefig('results\6dof\trajsgn_joint_vel.fig');
